...
// GEÄNDERT: Wir behandeln die Formularwerte als Strings, um Kommas zu erlauben.
const newProgressForms = ref<{ [key: string]: { weight: string; reps: string; sets: string } }>({})
  ...
  gymStore.categories.forEach(cat => {
    ...
    cat.exercises.forEach(ex => {
      if (!newProgressForms.value[ex.id]) {
        // GEÄNDERT: Initialisiert mit leeren Strings.
        newProgressForms.value[ex.id] = { weight: '', reps: '', sets: '' };
      }
    })
  })
})
...
function handleDeleteCategory(categoryId: string, categoryName: string) {
  if (window.confirm(`Möchtest du die Kategorie "${categoryName}" und alle darin enthaltenen Übungen wirklich löschen?`)) {
...
// GEÄNDERT: Diese Funktion wandelt Kommas in Punkte um und validiert die Eingaben.
function handleLogProgress(exerciseId: string) {
  const form = newProgressForms.value[exerciseId]
  if (!form || !form.weight || !form.reps || !form.sets) {
    alert("Bitte fülle alle Felder für den Log aus.");
    return;
  }
  ...
  // 3. Überprüfen, ob die Umwandlung erfolgreich war
  ...
  // 4. Fortschritt mit den korrekten Zahlen speichern
  gymStore.logProgress(exerciseId, { weight: weightNum, reps: repsNum, sets: setsNum })
    ...
    <div class="max-w-4xl mx-auto p-4 sm:p-6">
      ...
      <header class="mb-6">
        ...
        <p v-if="totalSets > 0" class="text-gray-600">
          Stark! Du hast heute schon <span class="font-bold text-blue-600">{{ totalSets }}</span> Sätze absolviert.
        </p>
        <p v-else class="text-gray-600">
          Zeit, ins Training zu starten!
        </p>
      </header>

      <div class="mb-8">
        <form @submit.prevent="handleAddCategory" class="flex gap-2 items-center">
          <input v-model="newCategoryName" type="text" placeholder="Neue Kategorie (z.B. Push, Pull...)"
      ...
      <div class="space-y-4">
        <div v-for="category in gymStore.categories" :key="category.id"
          ...
          <div @click="toggleCategory(category.id)"
            ...
            <div class="flex items-center gap-3">
              <button @click.stop="handleDeleteCategory(category.id, category.name)"
              ...
              <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-gray-400 transition-transform duration-300"
                :class="{ 'rotate-180': !collapsedCategories.includes(category.id) }" fill="none" viewBox="0 0 24 24"
                stroke="currentColor">
          ...
          <Transition>
            <div v-if="!collapsedCategories.includes(category.id)">
              <div class="px-4 pb-4 space-y-3">
                <div v-for="exercise in category.exercises" :key="exercise.id" class="border-t border-gray-100 pt-3">
                  <div class="flex justify-between items-start">
                    <div>
                      <h4 class="font-semibold text-lg text-gray-700">{{ exercise.name }}</h4>
                      <a v-if="exercise.videoUrl" :href="exercise.videoUrl" target="_blank" rel="noopener noreferrer"
                        class="text-sm text-blue-500 hover:underline">
                        Anleitung ansehen
                      </a>
                    </div>
                    <button @click="handleDeleteExercise(category.id, exercise.id, exercise.name)"
                  ...
                  <form @submit.prevent="handleLogProgress(exercise.id)"
                    class="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-3">
                    <input v-model="newProgressForms[exercise.id].weight" type="text" inputmode="decimal" step="any"
                      placeholder="kg"
                      class="p-2 border border-gray-200 rounded-lg focus:ring-1 focus:ring-blue-500 focus:outline-none w-full text-center">
                    <input v-model="newProgressForms[exercise.id].reps" type="text" inputmode="numeric" step="1"
                      placeholder="Wdh"
                      class="p-2 border border-gray-200 rounded-lg focus:ring-1 focus:ring-blue-500 focus:outline-none w-full text-center">
                    <input v-model="newProgressForms[exercise.id].sets" type="text" inputmode="numeric" step="1"
                      placeholder="Sätze"
                      class="p-2 border border-gray-200 rounded-lg focus:ring-1 focus:ring-blue-500 focus:outline-none w-full text-center">
                    <button type="submit"
                      class="col-span-2 sm:col-span-1 bg-blue-500 text-white font-semibold p-2 rounded-lg hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors">Log</button>
                  </form>

                  <div v-if="exercise.history.length > 0" class="mt-3">
                    <div @click="toggleHistory(exercise.id)"
                      class="flex justify-between items-center cursor-pointer p-2 -mx-2 rounded-lg hover:bg-gray-50">
                      <h5 class="font-semibold text-sm text-gray-600">Letzte Einträge ({{ exercise.history.length }})
                      </h5>
                      <svg xmlns="http://www.w3.org/2000/svg"
                        class="h-5 w-5 text-gray-400 transition-transform duration-300"
                        :class="{ 'rotate-180': !collapsedHistories.includes(exercise.id) }" fill="none"
                        viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                      </svg>
                    </div>
                    <Transition>
                      <ul v-if="!collapsedHistories.includes(exercise.id)" class="space-y-1.5 text-sm mt-2">
                        <li v-for="(entry, index) in exercise.history" :key="index"
                          class="flex justify-between items-center text-gray-600 bg-gray-50 p-2 rounded-lg">
                          <span>{{ formatDate(entry.date) }}</span>
                          <span class="font-mono text-gray-800 font-medium">{{ entry.weight }}kg &times; {{ entry.reps
                            }} &times; {{ entry.sets }}</span>
                        </li>
                      </ul>
                    </Transition>
                  </div>
                ...
                <div class="border-t border-gray-100 pt-3">
                  <div v-if="openExerciseFormId !== category.id">
                    <button @click="openExerciseFormId = category.id"
                      class="w-full text-left text-blue-600 font-semibold p-2 rounded-lg hover:bg-blue-50 transition-colors">
                      + Übung hinzufügen
                    </button>
                  </div>
                  <form v-else @submit.prevent="handleAddExercise(category.id)"
                    class="bg-blue-50 p-3 rounded-lg space-y-2">
                    <input v-model="newExerciseForms[category.id].name" type="text" placeholder="Name der Übung"
                      class="w-full p-2 border border-gray-200 rounded-lg focus:ring-1 focus:ring-blue-500 focus:outline-none">
                    <input v-model="newExerciseForms[category.id].videoUrl" type="text"
                      placeholder="Optional: Video-Link"
                      class="w-full p-2 border border-gray-200 rounded-lg focus:ring-1 focus:ring-blue-500 focus:outline-none">
                    <div class="flex gap-2">
                      <button type="submit"
                        class="w-full bg-blue-500 text-white font-semibold p-2 rounded-lg hover:bg-blue-600 transition-colors">Speichern</button>
                      <button @click="openExerciseFormId = null" type="button"
                        class="w-full bg-gray-200 text-gray-700 font-semibold p-2 rounded-lg hover:bg-gray-300 transition-colors">Abbrechen</button>
                    </div>
                  </form>
                </div>
              </div>
