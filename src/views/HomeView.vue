...
// --- Lifecycle Hooks ---
// Stellt sicher, dass die Formular-Objekte für alle Elemente existieren
watchEffect(() => {
  gymStore.categories.forEach((cat) => {
...
function handleAddExercise(categoryId: string) {
  ...
  form.name = ''
  form.videoUrl = ''
  openExerciseFormId.value = null // Formular nach Hinzufügen schließen
}
            ...
            >
              ...
              <div v-if="exercise.history.length > 0" class="mt-3">
                <ul class="space-y-1.5 text-sm">
                  <li
                    v-for="(entry, index) in exercise.history.slice(0, 3)"
                    :key="index"
                  ...
                  >
                    ...
                    <span class="font-mono text-gray-800 font-medium"
                      >{{ entry.weight }}kg &times; {{ entry.reps }} &times; {{ entry.sets }}</span
                    >
                  </li>
                </ul>
              </div>
